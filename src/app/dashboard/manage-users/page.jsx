...
export default function ManageUserPage() {
  ...
  const handleChangeRole = (id, currentRole) => {
    ...
    const confirmChange = window.confirm(`Are you sure you want to change this user's role to ${nextRole}?`);
    
    if (confirmChange) {
      setUsers(prev => prev.map(user => 
        user.id === id ? { ...user, role: nextRole } : user
      ));
      console.log(`User ${id} role updated to ${nextRole}`);
      // TODO: Connect your backend API patch route here (e.g., axios.patch(`/api/users/${id}`, { role: nextRole }))
    }
  };

  // 2. Functional Delete User Handler
  const handleDeleteUser = (id, name) => {
    const confirmDelete = window.confirm(`⚠️ WARNING: Are you sure you want to permanently delete user "${name}"?`);
    
    if (confirmDelete) {
      setUsers(prev => prev.filter(user => user.id !== id));
      console.log(`User ${id} has been permanently deleted.`);
      // TODO: Connect your backend API delete route here (e.g., axios.delete(`/api/users/${id}`))
    }
  };
